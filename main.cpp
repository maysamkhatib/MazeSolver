...
int moveCells(int x, int y, int x0, int y0, int dir, pair<int, int> (*parent)[SIZE], bool ok = 0){
    string path = "";
    int x00 = x0, y00 = y0;
...
int getPath(int x, int y, int xx, int yy, int direction, bool ok=false) {
    ...
    return moveCells(x, y, xx, yy, direction, parent, ok);
}
...
void exploreMaze() {
    ...
    if(path == previousPath){
        direction = getPath(SIZE-1, 0, SIZE-y-1, x, direction);
        API::clearAllColor();
        while(direction != 0){
            direction = (direction+1)%4;
            API::turnRight();
